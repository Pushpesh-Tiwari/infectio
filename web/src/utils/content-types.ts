...
const contentTypes: {
  ...
  tsx: {
    ...
    group: "code",
    description: "Typescript extended",
...
const getContentType = (label: string): ContentType => {
  const contentType = contentTypes[label];
  if (!contentType) {
    throw new Error(`Content type for label "${label}" does not exist.`);
  }
  return contentType;
};
